...
//Dom Start pasted from app.js merge

//TODO:collect data from the input and store it in Firebase.
//

var un = prompt("you name");
document.getElementById("btnLogin").innerHTML = un;
const yes = "block";
const no = "none";
let x;
let seed;
...
plantCalc.innerHTML = `<p class = "plantCount">Pick any seed to see near about yield per plant. As of now the calculator is evolving</p>`;
//seed collection
let vegSeedis = {
  bitterGourd: 500,
  okra: 200,
...
var seedSeason = () => {
  ...
  switch (x) {
    case "okra":
      plantCalc.innerHTML = `${plantProp} ${X} is near about <h2 class="largerX">${
        vegSeedis.okra
      }grams</h2>`;
      seed = vegSeedis.okra;
    ...
    case "bottleGourd":
      plantCalc.innerHTML = `${plantProp} ${x.toUpperCase()} is near about <h2 class="largerX">${
        vegSeedis.bottleGourd
      }grams</h2>`;
      //TODO calculation with variable unique to this.seed
    ...
    case "select seed":
      plantCalc.innerHTML = "Pick any seed from the list";
      alert("Pick any seed");
      break;
...
let member = () => {
  ...
  if (m <= 0) {
    ...
    return (document.getElementById("zero").textContent =
      "Oh! it is lonely here!");
  } else {
...
let avgConsumptionPerPerson = () => {
  ...
  (() => {
    document.getElementById(
      "zero1"
    ).textContent = `Average consumption per member is ${c}gm`;
  ...
  if (c <= 0 || c < 5) {
    ...
    return (document.getElementById(
      "zero1"
    ).textContent = `Is ${x.toUpperCase()} restricted diet for you?`);
  }
...
let numberOfTimesRepeated = () => {
  ...
  document.getElementById("zero2").innerHTML = `<i>${(m * c * r) /
    1000}</i> <sup>Kg is your consumption for the season!</sup>`;
...
let surroundingRates = () => {
  document.getElementById(
    "rn6"
  ).textContent = `Price difference of InOrganic Food * ${kgs} x enter price per Kg`;
};
...
let howManyPlants = () => {
  ...
  //calculation is done now we have the yield! let's see if plant calculation is fit for creating roof garden or at farms.
  if (y >= 1 && y <= 90) {
    return (function() {
    ...
    document.getElementById(
      "num"
    ).innerHTML = `${roofCapacityExceeded}<br>${reset} `;
    ...
    document.getElementById(
      "num"
    ).innerHTML = `<h2 class='nPlants'>You may Grow 1 plant for this season, take good care of yourself :) </h2>${reset}`;
  }
};
